...
2. Добавь `useEffect`, который при изменении `search / sort / formatFilter / speedRange`
   ...
   ```
3. Где держать результат — реши по текущей структуре:
   - если список рендерится только в LeftIsland — локальный `useState` в LeftIsland;
   - `appStore.tracks` (глобальный список для «Show all»/upload-обновлений) не перетирай
     фильтрованными результатами, если от него зависят другие компоненты — сначала
     проверь grep'ом потребителей `tracks` из appStore.
