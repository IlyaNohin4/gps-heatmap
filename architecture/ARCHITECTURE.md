...
distance_km, duration_sec, moving_time_sec, recorded_at, uploaded_at
speed_avg, speed_max, speed_min             [km/h; speed_avg = moving time based, T25]
elevation_gain, elevation_loss              [m; методика gpx.studio — RDP eps=20 по
                                              профилю дистанция/высота + скользящее
                                              среднее 0.1км, см. POLISH.md T26 audit]
...
POST   /api/auth/change-password       — смена пароля (проверяет old_password)
...
POST   /api/tracks/create              — создать трек из точек, нарисованных на карте
...
GET    /api/tracks/public/{public_token}— просмотр без авторизации. Отдаёт
       полный TrackDetail анониму, включая raw_points с точным временем
       каждой GPS-точки (то есть точные моменты "когда я был в этой точке
       маршрута" — паттерны вроде "во сколько вышел из дома" вычисляемы).
       Осознанный trade-off: публичная ссылка задумана как полноценный
       шаринг трека (тот же уровень детализации, что видит сам автор), не
       урезанная версия. Кто не хочет светить таймстампы — не публикует
       трек или ротирует токен (publish/rotate) после просмотра.
...
POST   /api/poi/create                 — создать одну точку. body требует import_name
       (обязательное поле, min_length=1) — точка сразу попадает в список
       (существующий или новый, автовивифицируется через
       _get_or_create_import, см. § POIImport)
...
1. **Parse + Normalize** (см. PARSER.md)
   ...
   - Расчет метрик: distance, speed (moving time, T25), elevation_gain/loss
     (gpx.studio-методика, T26), grade_stats (climbing/descent/flat %) —
     **`grade_stats` считается парсером, но нигде не персистится и не отдаётся
     через API** (`process_track.py` его отбрасывает); фронтенд-график Slope
     (BottomIsland) вычисляет наклон сам, из `elevation`-точек
     `normalized_points`, не из `grade_stats`
...
3. **Save to DB**
   ...
   - `speed_segments` (включает grade_percent/type на пару точек),
     `moving_time_sec`, `elevation_gain/loss` — расчеты из normalized_points
...
- test_auth.py — регистрация, логин, сброс пароля, смена пароля, удаление аккаунта
- test_tracks.py — загрузка 5 форматов, изоляция по user
- test_parser.py — speed_segments, нормализация, grade_stats
- test_elevation_methodology.py — RDP/windowed-average функции gpx.studio-методики (T27)
- test_elevation_smoothing.py — Savitzky-Golay сглаживание (Phase 4)
- test_grade_classification.py — climbing/descent/flat классификация (Phase 5)
- test_trajectory_simplification.py — Douglas-Peucker упрощение (Phase 6)
- test_process_track_retry.py — Celery retry/backoff политика (T09)
- test_poi.py, test_poi_parser.py — CRUD и парсинг импорта POI
- test_db_integration.py — реальный Postgres, savepoint-изоляция
- test_security.py — XSS, размер файла, невалидный формат
- **Coverage:** 162/162 tests ✓ (`docker compose exec backend python -m pytest --collect-only -q`)
